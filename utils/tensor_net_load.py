...
def init(H, config=None):
    ...
    graph_def_orig_file = '%s/../data/classify_image_graph_def.pb' % os.path.dirname(os.path.realpath(__file__))
    ...
    weights_ops = [
        ...
        if any(op.name.endswith(x) for x in [ '_params'])
        and op.type == 'Const'
    ]
