...
def init(H, config=None):
    ...
    k = H['num_classes']
    features_dim = 1280
    input_layer = 'ResizeBilinear'

    features_layers = ['output/confidences', 'output/boxes']

    graph_def_orig_file = '%s/../data/inception3_merge_10_16.pb' % os.path.dirname(os.path.realpath(__file__))
    ...
    weights_ops = [
        ...
        if any(op.name.endswith(x) for x in [ 'weights', 'biases'])
        and op.type == 'Const'
    ]
    ...
    reuse_ops = [
        ...
        and op.name != 'inception_v3_3/logits/predictions'
    ]
...
def model(x, googlenet, H):
    ...
    coarse_feat = T['inception_v3/mixed_17x17x1280a/concat']

    # fine feat can be used to reinspect input
    attention_lname = H.get('attention_lname', 'inception_v3/logits/flatten/Reshape')
