...
from utils import train_utils, googlenet_load,inception3_load
...
def build_overfeat_inner(H, lstm_input):
    ...
    with tf.variable_scope('Overfeat', initializer=initializer):
        w = tf.get_variable('ip', shape=[2048, H['lstm_size']])
...
def build_forward(H, x, googlenet, phase, reuse):
    ...
    input_mean = 117.
    x -= input_mean
    cnn, early_feat, _ = inception3_load.model(x, googlenet, H)
    ...
    cnn = tf.reshape(cnn,
                     [H['batch_size'] * H['grid_width'] * H['grid_height'], 2048])
    initializer = tf.random_uniform_initializer(-0.1, 0.1)
    with tf.variable_scope('decoder', reuse=reuse, initializer=initializer):
        scale_down = 0.01
        lstm_input = tf.reshape(cnn * scale_down, (H['batch_size'] * grid_size, 2048))
        if H['use_lstm']:
...
def build(H, q):
    ...
    encoder_net = inception3_load.init(H, config)
